# Hopa în Grădină — reguli pentru agenți
...
Site-ul îl folosește și îl editează proprietarul Hopa în Grădină — care **nu este programator și nu este designer**. Cunoaște businessul, nu codul.
...
Site pentru Hopa în Grădină — grădină de evenimente outdoor în Mogoșoaia, pentru aniversări, serbări, petreceri private și de echipă. Conversia principală a siteului este **rezervarea unui eveniment** (lead → telefon / email / formular). Mai multe în `BRIEF.md`.
...
Pagini: `index.html` (pagina principală) + ghidul de stil `stylesheet.html`, stil în `styles.css`.

Hopa în Grădină are propriul nume de domeniu, `hopaingradina.ro`. Site-ul surori, Hopa Țopa Land (indoor, Colentina), e un repo separat — link-ul către el (`hopatopaland.ro`) rămâne ca buton de încrucișare pe pagina principală.
...
Dacă ți se cere să construiești ceva care **nu există în ghidul de stil** (o componentă nouă, o culoare nouă, un layout sau un efect care nu apare în `STYLEGUIDE.md` / `styles.css`):
